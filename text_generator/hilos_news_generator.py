...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', required=True, type=str, help='Prefijo del nombre del modelo')
    parser.add_argument('--id_llm', required=True, type=int, help='ID numérico del LLM')
    ...
    parser.add_argument('--api_ip', default="localhost", help="IP API Backend de periódicos")
    parser.add_argument('--api_port', type=int, default=5010, help="Puerto API Backend")
    
    parser.add_argument('--chroma_ip', default="localhost", help="IP ChromaDB")
    parser.add_argument('--chroma_port', type=int, default=8001, help="Puerto ChromaDB")
    
    parser.add_argument('--ollama_ip', default="localhost", help="IP Ollama")
    parser.add_argument('--ollama_port', type=int, default=11434, help="Puerto Ollama")
    
    parser.add_argument('--llm_api_ip', default="localhost", help="IP API LLM Wrapper")
    parser.add_argument('--llm_api_port', type=int, default=6002, help="Puerto API LLM Wrapper")

    args = parser.parse_args()

    # --- 2. Configuration Setup ---
    API_IP = args.api_ip
    REAL_API_PORT = args.api_port
    CHROMADB_IP = args.chroma_ip
    CHROMADB_PORT = args.chroma_port
    OLLAMA_IP = args.ollama_ip
    OLLAMA_PORT = args.ollama_port
    LLM_API_IP = args.llm_api_ip
    LLM_API_PORT = args.llm_api_port

    print("=== CONFIGURACIÓN DE RED ACTIVA ===")
    print(f"API Noticias: {API_IP}:{REAL_API_PORT}")
    print(f"ChromaDB:     {CHROMADB_IP}:{CHROMADB_PORT}")
    print(f"Ollama:       {OLLAMA_IP}:{OLLAMA_PORT}")
    print(f"API Guardado: {LLM_API_IP}:{LLM_API_PORT}")
    print("===================================")
